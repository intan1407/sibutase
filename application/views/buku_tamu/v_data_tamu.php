...
<script>
	...
	function createTable(result,sno){
		...
		for(index in result){
			...
			if(photo != ""){
				var foto = '<img width="100" height="100" class="profile-user-img img-responsive" src="<?php echo base_url()?>assets/images/foto_tamu/'+photo+'" onclick="modalimage(this.src)">';
			}else {
			...
			tr += "<a href='<?php echo base_url()?>data_buku_tamu/get_edit_tamu/"+id+"' class='btn btn-primary btn-edit' data-placement='top' data-tooltip='tooltip' title='Edit' ><span class='fa fa-edit'></span></a>&nbsp;";
			tr += "<a class='btn btn-danger btn-hapus' data-toggle='modal' data-id='"+id+"' data-foto='"+photo+"' data-target='#ModalHapus'  data-placement='left'  data-tooltip='tooltip' title='Hapus' ><span class='fa fa-trash'></span></a></td>";
			tr += "</tr>";
