        ...
        <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-body">
            	...
            	<table class="table table-bordered table-striped" id='guruList' style="font-size:13px;">
            		<thead>
	                	<tr>
		          				<th>No.</th>
		          				<th>Nip</th>
  ...
  <!--Modal Add Guru-->
	<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
	    <div class="modal-dialog" role="document">
	        <form class="form-horizontal" action="<?php echo base_url().'guru/simpan_guru'?>" method="post" enctype="multipart/form-data">
	        <div class="modal-content">
	            <div class="modal-header">
	                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><span class="fa fa-close"></span></span></button>
	                <h4 class="modal-title" id="myModalLabel">Add Data Guru</h4>
	            </div>
	...
	<div class="modal fade" id="ModalEdit" tabindex="-1" role="dialog" aria-labelledby="ModalEditLabel">
	    <div class="modal-dialog" role="document">
	        <form class="form-horizontal" action="<?php echo base_url().'guru/update_guru'?>" method="post" enctype="multipart/form-data">
	        <div class="modal-content">
	            <div class="modal-header">
	                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true"><span class="fa fa-close"></span></span></button>
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				...
				<div class="modal-body">
					...
					<p>Anda yakin akan menghapus data guru ini?</p>
				</div>
				<div class="modal-footer">
				 	<button type="button" class="btn btn-primary btn-flat hapus" id="hapus_guru">Ya, Hapus!</button>
				 	<button type="button" class="btn btn-default btn-flat" data-dismiss="modal">Batal</button>
				</div>
			</div>
		</div>
...
<script>
	...
	function loadPagination(pagno){
		$.ajax({
			url: '<?php echo base_url()?>guru/loadRecord/'+pagno,
			type: 'get',
			dataType: 'json',
	...
	function createTable(result,sno){
		...
		for(index in result){
			...
			tr += "&nbsp;<a class='btn btn-warning btn-hapus' data-toggle='modal' data-id="+id+" data-target='#ModalHapus'><span class='fa fa-trash'></span></a>";
			tr +="</td>";
			tr += "</tr>";
			$('#guruList tbody').append(tr);
		}
		...
		$(".btn-edit").on("click", function() {
			...
			$.ajax({
				type: 'POST',
				dataType: 'json',
				url: '<?php echo base_url('guru/get_data_edit_guru')?>',
				data:{id:id},
				success: function(res){
					var guru =res.guru[0]; 
					$('#id_update').val(guru.id);
					$('#nip_update').val(guru.nip);
					$('#nama_update').val(guru.gurunama);
					$('#jabatan_update').val(guru.jabatan).change();;
					$("#bagian_update").val(guru.bagian).change();;
					$('#no_hp_update').val(guru.no_hp);
					$('#status_update').val(guru.status);
	...
	$("#hapus_guru").on("click", function(){
		...
		$.ajax({
			type: 'POST',
			dataType: 'json',
			url: '<?php echo base_url('guru/hapus_guru')?>',
			data:{id:id},
			success: function(res){
				pageno = $("#pagenomer").val();
				loadPagination(pageno);
	...
	$(".filter").on("input", function() {
		...
		$.ajax({
			type: 'POST',
			dataType: 'json',
			url: '<?php echo base_url('guru/filter_data')?>',
	...
	function loadbagian(){
		$.ajax({
			url: '<?php echo base_url()?>guru/loadbagian',
			type: 'get',
			success: function(response){
	...
	function loadjabatan(){
		$.ajax({
			url: '<?php echo base_url()?>guru/loadjabatan',
			type: 'get',
			success: function(response){
