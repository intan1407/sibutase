...
class M_tamu extends CI_Model{
	...
	*/
	function get_tamu_perpage($offset,$limit){
		$this->db->select('tbl_tamu.*,tbl_guru.id as id_guru,tbl_guru.nama as namatujuan ');
		$this->db->from('tbl_tamu');
		$this->db->join('tbl_guru','tbl_guru.id = tbl_tamu.nama_tujuan','LEFT');
	...
	function get_filter_tamu_perpage($offset,$limit,$keywords){
		$this->db->select('tbl_tamu.*,tbl_guru.id as id_guru,tbl_guru.nama as namatujuan ');
		$this->db->from('tbl_tamu');
		$this->db->join('tbl_guru','tbl_tamu.nama_tujuan = tbl_guru.id','LEFT');
		...
		$this->db->or_like('tbl_guru.nama',$keywords);
	...
	function get_guru(){
        ...
        $hsl = $this->db->get('tbl_guru')->result();		
		return $hsl;
	}
	function get_bagian(){
